...
async function apiCall(endpoint, method = 'GET', body = null) {
    ...
    try {
        ...
        data = { error: 'ไม่สามารถอ่านข้อมูลจากเซิร์ฟเวอร์ได้' };
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 💡 ฟังก์ชันใหม่: ดักจับการกด Enter ในช่องกรอกข้อมูลเพื่อ Login
    ...
    document.getElementById('check-reset-status-btn')?.addEventListener('click', handleForgotPasswordCheck);
    document.getElementById('submit-new-password-btn')?.addEventListener('click', handlePerformReset);
    ...
    document.getElementById('stock-image-upload')?.addEventListener('change', handleImageFileSelect);
    ...
    document.getElementById('import-users-btn')?.addEventListener('click', handleImportUsers);
    ...
    document.getElementById('admin-dashboard-panel')?.addEventListener('click', (e) => {
        ...
        if (filterTarget) {
            handleStockFilter(filterTarget);
        }
    });
...
// --- Event Delegation (สำหรับปุ่มที่ถูกสร้างผ่าน JavaScript แบบ Dynamic) ---
document.addEventListener('click', async function(e) {
    ...
    if (e.target.matches('.approve-btn')) {
        ...
        if (isNaN(approvedQuantity) || approvedQuantity <= 0 || approvedQuantity > originalQuantity) {
            showNotification(`จำนวนที่อนุมัติต้องอยู่ระหว่าง 1 ถึง ${originalQuantity}`, 'error');
            return;
        }
        if (approvedQuantity < originalQuantity && !reason) {
            showNotification('กรุณากรอกเหตุผลที่ลดจำนวน', 'error');
            reasonInput.focus();
            return;
        }
        ...
        try {
            await apiCall('/api/admin/approve', 'POST', { requestId: id, approvedQuantity, reason, adminUser: currentUser.username });
    ...
    if (e.target.matches('.process-return-btn')) {
        ...
        if (!returnConditionEl || !disbursementTypeEl) {
            showNotification('กรุณาเลือกตัวเลือกให้ครบถ้วน', 'error');
            return;
        }
    ...
    // ==========================================
    // 📦 การจัดการสต๊อกแบบใหม่ (รับเข้า, ปรับปรุง, ประวัติ)
    // ==========================================
    
    // 1. รับของเข้า (Receive Stock)
    if (e.target.matches('.receive-stock-btn')) {
        ...
        showPromptModal(`รับของเข้า: ${type} (${size})\nกรอกจำนวนที่รับเข้า (ชิ้น):`, async (qtyStr) => {
            ...
            showLoadingButton(e.target, true);
            try {
                await apiCall('/api/stock/transaction', 'POST', { 
                    itemType: type, size: size, transactionType: 'IN', quantity: qty, 
                    reason: 'รับพัสดุเข้าใหม่', adminUser: currentUser.username 
                });
        ...
        showPromptModal(`ปรับปรุงยอดของใหม่: ${type} (${size})\nกรอก "ยอดคงเหลือสุทธิ" ที่นับได้:`, async (qtyStr) => {
            ...
            setTimeout(() => {
                showPromptModal(`เหตุผลในการปรับปรุงยอด:`, async (reason) => {
                    ...
                    try {
                        await apiCall('/api/stock/transaction', 'POST', { 
                            itemType: type, size: size, transactionType: 'ADJUST', newBalance: qty, 
                            reason: reason, adminUser: currentUser.username 
                        });
    ...
    if (e.target.matches('.history-stock-btn')) {
        ...
        try {
            ...
            history.forEach(log => {
                let badgeColor = log.transactionType === 'IN' ? 'bg-emerald-100 text-emerald-700' : 
                                 log.transactionType === 'OUT' ? 'bg-rose-100 text-red-700' : 'bg-amber-100 text-amber-700';
                html += `
                <li class="p-3 bg-slate-50 rounded-lg border border-slate-100 flex justify-between items-start">
                    <div>
                        <p class="font-bold text-slate-800 text-sm"><span class="px-2 py-0.5 rounded-md text-[10px] ${badgeColor}">${log.transactionType}</span> ${log.quantity > 0 ? '+'+log.quantity : log.quantity} ชิ้น</p>
                        <p class="text-xs text-slate-500 mt-1">เหตุผล: ${log.reason || '-'}</p>
                        <p class="text-[10px] text-slate-400 mt-1">ทำรายการโดย: ${log.adminUser}</p>
                    </div>
                    <span class="text-[10px] font-bold text-slate-400 bg-white px-2 py-1 rounded border">${new Date(log.createdAt).toLocaleString()}</span>
                </li>`;
            });
            html += '</ul>';
            document.getElementById('history-modal-content').innerHTML = html;
        } catch(e) {
            document.getElementById('history-modal-content').innerHTML = '<p class="text-center text-red-500 mt-4">ไม่มี API รองรับ หรือ เกิดข้อผิดพลาด</p>';
        }
    }
    ...
    // Admin: รีเซตพาสเวิร์ดผู้ใช้ (Reset Password)
    ...
    // การเปิด Modal ประวัติการใช้งานของผู้ใช้
        ...
        try {
            ...
            document.getElementById('history-modal-content').innerHTML = `
                <div class="grid grid-cols-2 gap-4 text-center mt-4">
                    <div class="bg-indigo-50 p-4 rounded-xl border border-indigo-100"><p class="text-sm font-bold text-indigo-800">คำขอทั้งหมด</p><p class="text-3xl font-black text-indigo-600">${summary.length || 0}</p></div>
                    <div class="bg-emerald-50 p-4 rounded-xl border border-emerald-100"><p class="text-sm font-bold text-emerald-800">พัสดุที่เคยเบิก</p><p class="text-3xl font-black text-emerald-600">${totalItems}</p></div>
                </div>`;
...
function onLoginSuccess(user) {
    ...
    if (user.mustChangePassword) {
        currentUser = user; // จำไว้ก่อนเพื่อทำการเปลี่ยนพาสเวิร์ด
...
async function loadInitialData() {
    ...
    try {
        const [stockData, requestsData, holdingsData] = await Promise.all([
            apiCall('/api/stock'),
            apiCall(`/api/requests/me?username=${currentUser.username}`),
            apiCall(`/api/requests/holdings?username=${currentUser.username}`)
        ]);
...
async function loadAdminData() {
    try {
        const [pendingReqs, users, logs] = await Promise.all([
...
function onStockReceived(newStockData) {
    ...
    if (currentUser && currentUser.role === 'admin') {
        // ใช้ตัวกรองปัจุบันถ้ามีการเลือกไว้
        const activeFilter = document.querySelector('.stock-filter-btn.bg-indigo-600');
        if(activeFilter) {
            handleStockFilter(activeFilter);
        } else {
            displayStockSummary(masterStock);
        }
        
        // ตรวจสอบ Stock ต่ำสำหรับแอดมิน
...
function populateSizeDropdown() {
    ...
    const availableItems = masterStock.filter(item => item.itemType === selectedType);
    sizeSelect.innerHTML = '<option value="">-- เลือกขนาด --</option>';
    
    availableItems.forEach(item => {
        if(item.size) sizeSelect.add(new Option(`${item.size} (คงเหลือ: ${item.newStock} ชิ้น)`, item.size));
    });
...
function displaySelectedItemImage() {
    ...
    const size = document.getElementById('request-size').value;
    const previewContainer = document.getElementById('item-image-preview-container');
    const previewImage = document.getElementById('item-image-preview');

    const stockItem = masterStock.find(item => item.itemType === type && item.size === size);
    if (stockItem && stockItem.imageUrl) {
        previewImage.src = stockItem.imageUrl;
...
// 2. ข้อมูลการคืน (Return dropdown)
function populateReturnableItemsDropdown() {
    ...
    if (userApprovedItems && userApprovedItems.length > 0) {
        ...
        select.innerHTML = '<option value="">-- เลือกรายการที่เคยเบิก --</option>';
        userApprovedItems.forEach(item => {
            select.add(new Option(`${item.itemType} (ไซส์ ${item.size}) - คงเหลือ ${item.quantity} ชิ้น`, item.requestId));
        });
    } else {
        select.disabled = true;
        select.add(new Option('ไม่พบรายการที่สามารถคืนได้', ''));
    }
}
...
function displayRequests(requests, tableId, isAdminView) {
    ...
    if (!requests || requests.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="${colspan}" class="text-center p-4 text-gray-500">ไม่พบข้อมูล</td></tr>`;
        return;
    }

    requests.forEach(req => {
        const { requestId, createdAt, requesterName, itemType, size, quantity, status, notes } = req;
        const tr = document.createElement('tr');
        
        let actionButton = '-';
        if (!isAdminView && status === 'Approved' && quantity > 0) {
            actionButton = `<span>สามารถคืนได้ในฟอร์มด้านซ้าย</span>`;
        }
        
        const safeStatus = (status || 'unknown').replace(' ', '-').toLowerCase();
        ...
        const requesterCell = isAdminView ? `<td class="p-2 text-sm text-gray-700 font-medium">${requesterName}</td>` : '';

        tr.innerHTML = `
            <td class="p-2 text-sm text-gray-600 whitespace-nowrap">${new Date(createdAt).toLocaleString()}</td>
            ${requesterCell}
            <td class="p-2 text-sm font-medium text-gray-800">${itemType} (ไซส์ ${size}) x ${quantity}</td>
            <td class="p-2 text-sm"><span class="px-2 py-1 font-semibold leading-tight text-xs rounded-full ${statusClass}">${status}</span></td>
            <td class="p-2 text-xs text-gray-500">${notes || '-'}</td>
...
function displayCurrentUserHoldings(holdings) {
    ...
    if (!holdings || Object.keys(holdings).length === 0 || holdings.error) {
        listDiv.innerHTML = `<p class="text-center text-gray-500">คุณไม่มีพัสดุที่ถือครองอยู่</p>`;
        return;
    }
    ...
    for (const item in holdings) {
        ul.innerHTML += `<li class="flex justify-between items-center text-sm bg-gray-50 p-2 rounded"><span class="font-medium text-gray-700">${item}</span><span class="font-bold text-gray-900">${holdings[item]} ชิ้น</span></li>`;
    }
    listDiv.appendChild(ul);
}

// 4. หน้า Admin: รออนุมัติ
function displayPendingApprovals(requests) {
    ...
    if (!requests || requests.length === 0) {
        list.innerHTML = '<p class="text-center p-4 text-gray-500">ไม่มีรายการรออนุมัติ</p>';
        return;
    }
    ...
    requests.forEach(req => {
        ...
        card.className = 'border rounded-lg p-4 bg-gray-50 approval-card shadow-sm border border-slate-200';
        let content;
        ...
        if (status === 'Pending Return') {
            ...
            content = `
                ...
                <div class="grid grid-cols-5 gap-4 mt-4 pt-4 border-t border-slate-200">
                    <div class="col-span-2"><label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">จำนวนที่อนุมัติให้เบิก</label><input type="number" value="${qty}" min="1" max="${qty}" class="approval-quantity-input w-full py-2 px-3 border border-slate-200 rounded-lg text-sm font-bold text-center focus:ring-2 focus:ring-indigo-500 outline-none"></div>
                    <div class="col-span-3"><label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">หมายเหตุให้ผู้เบิก</label><input type="text" placeholder="เช่น สต็อกไม่พอ..." class="approval-reason-input w-full py-2 px-3 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"></div>
                </div>
...
function displayStockSummary(stockData) {
    ...
    if (!stockData || stockData.length === 0) {
        container.innerHTML = '<p class="text-center p-4 text-gray-500">ไม่พบข้อมูลสต็อก</p>';
        return;
    }
    ...
    for (const category in groupedByCategory) {
        ...
        table.innerHTML = `
            <thead class="bg-slate-50"><tr>
                ...
                <th>รายการพัสดุ</th>
                <th class="text-center">ยอดของใหม่</th>
                <th class="text-center">ยอดรวมอื่นๆ</th>
                <th class="text-center">จัดการสต๊อก (Ledger)</th>
            </tr></thead>
        ...
        groupedByCategory[category].forEach(item => {
            ...
            const img = item.imageUrl || 'https://placehold.co/80x60/e2e8f0/64748b?text=N/A';
            tr.innerHTML = `
                <td class="p-2 text-center"><img src="${img}" class="w-12 h-12 object-cover rounded-lg mx-auto"></td>
                <td class="p-3">
                ...
                <td class="p-3 text-center">
                    <span class="text-lg font-black text-emerald-600">${item.newStock}</span>
                </td>
                <td class="p-3 text-center">
                    <p class="text-[10px] text-slate-500 font-bold">มือสอง: <span class="text-blue-600 text-xs">${item.usedStock}</span></p>
                    <p class="text-[10px] text-slate-500 font-bold">ชำรุด: <span class="text-red-600 text-xs">${item.damagedStock}</span></p>
                </td>
                <td class="p-3 text-center">
...
async function handleCreateNewStockItem() {
    ...
    const data = {
        ...
        newStock: parseInt(document.getElementById('stock-new-qty').value) || 0,
        usedStock: parseInt(document.getElementById('stock-used-qty').value) || 0,
        damagedStock: parseInt(document.getElementById('stock-damaged-qty').value) || 0,
        lowStockThreshold: parseInt(document.getElementById('stock-threshold-qty').value) || 5,
        imageUrl: document.getElementById('stock-image-url').value,
    ...
    try {
        ...
        onAdminActionSuccess('สร้างรายการพัสดุสำเร็จ');
...
function handleStockFilter(clickedButton) {
    ...
    let filteredStock = masterStock;
    if (clickedButton.id === 'stock-filter-low') {
        filteredStock = masterStock.filter(item => item.newStock > 0 && item.newStock <= (item.lowStockThreshold || 5));
    }
    displayStockSummary(filteredStock);
}
...
function populateStockForm(type, size) {
    ...
    if (item) {
        ...
        if(preview) preview.src = item.imageUrl || 'https://placehold.co/128x128/e2e8f0/64748b?text=Preview';
        ...
        document.getElementById('stock-type').disabled = true;
        document.getElementById('stock-size').disabled = true;
...
function clearStockForm() {
    ...
    const preview = document.getElementById('stock-image-preview');
    if(preview) preview.src = 'https://placehold.co/128x128/e2e8f0/64748b?text=Preview';
}
...
function renderUsersTable() {
    ...
    if(container) {
        container.innerHTML = `<table class="min-w-full divide-y divide-gray-200"><thead class="bg-slate-50"><tr>
            ...
            </tr></thead><tbody class="bg-white divide-y divide-slate-100"></tbody></table>`;
        ...
        pageData.forEach(user => {
            ...
            tbody.innerHTML += `<tr>
                ...
                <td class="p-3 text-sm text-slate-500">${user.department || '-'}</td>
                <td class="p-3 text-center"><span class="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider">${user.role}</span></td>
                ...
                <td class="p-3 text-center space-x-1.5 whitespace-nowrap">
                    <button class="edit-user-btn bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold py-1.5 px-3 rounded-lg transition-colors" data-username="${user.username}">แก้ไข</button>
                    <button class="reset-password-btn bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 text-xs font-bold py-1.5 px-3 rounded-lg transition-colors" data-username="${user.username}">รหัส</button>
                    <button class="delete-user-btn bg-rose-50 hover:bg-rose-100 border border-rose-200 text-red-700 text-xs font-bold py-1.5 px-3 rounded-lg transition-colors" data-username="${user.username}">ลบ</button>
                </td></tr>`;
        });
    }
...
async function handleSaveUser() {
    ...
    try {
        await apiCall('/api/users', 'POST', { userData, adminUser: currentUser.username });
...
async function handleImportUsers() {
    showNotification('ฟีเจอร์นำเข้าข้อมูลจะพร้อมใช้งานในเร็วๆ นี้', 'error');
}

// ==========================================
// 📊 ประวัติและ Logs (History)
// ==========================================
function onAdminLogReceived(logs) {
    allAdminLogData = logs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    handleHistoryViewChange(); 
}

function handleHistoryViewChange() {
    const view = document.getElementById('history-view-selector')?.value;
    if (view === 'requests') {
...
function renderAllHistoryTable() {
    ...
    const totalPages = Math.ceil(filteredData.length / rowsPerPage);
    if (currentHistoryPage > totalPages) currentHistoryPage = Math.max(1, totalPages);
...
function renderAdminLogTable() {
    ...
    const filteredData = allAdminLogData.filter(log => 
    ...
    const totalPages = Math.ceil(filteredData.length / rowsPerPage);
    if (currentLogPage > totalPages) currentLogPage = Math.max(1, totalPages);
    ...
    pageData.forEach(log => {
        tbody.innerHTML += `<tr>
            <td class="p-3 text-sm">${new Date(log.createdAt).toLocaleString()}</td>
...
async function handleExportHistory() { showNotification('ฟีเจอร์ Export จะใช้ได้เมื่อเชื่อมต่อฟังก์ชันรายงานหลังบ้าน', 'success'); }

// ==========================================
// 🔔 UI, Modals & Tabs Utilities
// ==========================================
function toggleRequestForm() {
    const reason = document.getElementById('request-reason-type').value;
    document.getElementById('return-quantity-wrapper')?.classList.add('hidden');
    if (reason === 'Damaged/Lost') {
...
function toggleMyHistory() {
    ...
    document.getElementById('my-history-toggle-icon').style.transform = content.classList.contains('expanded') ? 'rotate(0deg)' : 'rotate(-180deg)';
}

function toggleMyRequests() {
    const content = document.getElementById('my-requests-content');
    content?.classList.toggle('expanded');
    content.style.maxHeight = content.classList.contains('expanded') ? content.scrollHeight + 'px' : '0px';
    document.getElementById('my-requests-toggle-icon').style.transform = content.classList.contains('expanded') ? 'rotate(0deg)' : 'rotate(-180deg)';
}

function toggleStockForm(forceOpen = false) {
    const content = document.getElementById('stock-form-content');
    if (forceOpen) content.classList.add('expanded'); else content?.classList.toggle('expanded');
    content.style.maxHeight = content.classList.contains('expanded') ? content.scrollHeight + 'px' : '0px';
    document.getElementById('stock-form-toggle-icon').style.transform = content.classList.contains('expanded') ? 'rotate(0deg)' : 'rotate(-180deg)';
}
...
function resetActionButtons() {
    ...
    showLoadingButton(document.getElementById('update-stock-btn'), false, 'บันทึกข้อมูล');
...
function showPromptModal(title, callback) {
    ...
    document.getElementById('prompt-modal-submit-btn').onclick = () => {
        if(input.value.trim()) { closeModalAnimation(modal); callback(input.value.trim()); }
    };
...
async function handleForgotPasswordCheck() { showNotification('กรุณาติดต่อผู้ดูแลระบบเพื่อรีเซ็ตรหัสผ่าน', 'success'); }
